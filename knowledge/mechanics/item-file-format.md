...
2. **Multiple Talend-internal versions** (e.g. `i430_..._0.1.item`, `i430_..._0.2.item`) may coexist. These are *Talend Studio internal versions*, **not git versions**. Rule: the **highest version number is normally the active one**.
...
In user-facing prose (chat replies, documentation), refer to a job by its **job name only** — `i430_order_status_file_staging`, no `_0.1.item` suffix. The version is internal Talend metadata that distracts the reader.
...
Example body text:
> "The worker job `i430_order_status_file_staging` writes to the staging table…"

Example citation table at the end:

| Job | File |
|---|---|
| `i430_order_status_file_staging` | `<project>/process/i430_order_status_file_import/worker/i430_order_status_file_staging_0.1.item` |
