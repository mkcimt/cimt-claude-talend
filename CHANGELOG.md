...
- **`knowledge/patterns/job-instance-framework.md`** — the `tJobInstanceStart` / `tJobInstanceEnd` pattern (Layer 2b): the fixed pre/post-job skeleton, `JOB_INSTANCE_STATUS` / `JOB_INSTANCE_COUNTERS`, the three published globalMap values, and the `META_BATCH_ID > PREV_JOB_INSTANCE_ID AND <= JOB_INSTANCE_ID` window with its restart semantics. Documents per-work-item pointers (what makes a shared helper job safe for many callers — and the audit item when the work item is left empty) and the two-connections trap: `tJobInstanceEnd` closes the bookkeeping connection, so the `tDBClose` next to it belongs to the data connection.
- **`knowledge/patterns/scd-dispatcher-worker.md`** — the caller-parametrized SCD variant (Layer 2b), alongside the existing meta-table-driven `dynamic-scd-framework.md`: dynamic-schema worker, the two subjobs in one transaction, `tHashRow` hash computation in the extract job (and why the extract tMap leaves `META_HK`/`META_HV` empty), the tombstone `META_HV` mutation that makes resurrect-after-delete work, and the current-view caveats — tombstones are not filtered out, and "one row per key" assumes `META_LOAD_DATETIME` is unique within a key. Includes an audit checklist and a trade-off comparison against the meta-table variant.
...
- **`knowledge/INDEX.md` now lists every `patterns/` file.** `dynamic-scd-framework.md` had been in the tree but not in the index since it was added; the two new pattern files are listed alongside it. `dynamic-scd-framework.md` also gained a pointer to the caller-parametrized variant so the two SCD files find each other from either direction.
