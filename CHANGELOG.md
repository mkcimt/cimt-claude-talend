...
- **`CONTRIBUTING.md` — the contribution route is now spelled out.** "Fork or branch" left open the one thing that actually trips people up: the kit checkout a project points at is a *downstream* clone you cannot push from, so contributions need a second checkout of a personal fork. Adds the two-checkouts distinction, one-time fork setup, the rule that the fork's `main` stays a mirror of upstream, a Claude/user split for the per-change flow (Claude branches, commits and pushes; the user opens the PR, because Claude has no GitHub session), the base/head direction to verify on a cross-fork PR, and which token scopes a push to one's own fork actually needs. Also adds BACKLOG upkeep to the numbered checklist.
