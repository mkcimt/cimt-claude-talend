...
- **`knowledge/tmc/versioning.md`** — the three independent version axes that all get called "version" and are constantly confused: the Studio **job version** (manual; compiled into the generated Java package `<project>.<job>_0_1`, so readable from a stack trace), the published **artifact version** (`semver.buildTimestamp`, one immutable version per publish; semver seeded from the job version but editable, so the two need not match), and the server-side **task version** (a revision counter that bumps on every bind/config change and must be dropped on `PUT`). Linked from `INDEX.md`. Verified on TMC EU R2025-10.
