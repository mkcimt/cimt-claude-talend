...
- **`/project-intake` — deterministic static-review findings dimension.** `tools/talend_findings.py` adds a *breadth* review pass over each parsed `.item`: cheap, project-wide, high-precision heuristics — reload-at-each-row tMap lookups and SQL perf/smells (`SELECT *`, leading-wildcard `LIKE '%…'`, dynamically concatenated SQL), inactive/dead components left in a job, and missing error handling on a job that writes externally (no `tDie`/`tWarn`/`tLogCatcher` and no reject flow). Each finding carries `severity`, `category`, `detail`, `location`, and `provenance:"static"`; the deterministic Tier-1 catalog emits `perf`/`smell`/`dead_code` (the `bug` severity is reserved for the Tier-2 `talend-code-reviewer` depth pass). A `summarize()` rollup tallies by severity/category. Precision over recall by design — hard semantic bugs (guard completeness, auth bypass, …) are *not* decided here. This is the breadth tier of a two-tier triage: artifacts flagged for the depth pass are handed to the `talend-code-reviewer` agent for the semantic review (`knowledge/code-review/principles.md`), mirroring the complexity model's LLM-review hook. Offline `unittest` coverage in `tests/test_talend_findings.py`.
- **`/project-intake` phase 3 — read-only TMC enrichment + dependency/upgrade-risk dimension.** With `--tmc`, the analyzer now populates the canonical JSON's reserved `tmc{}` block from the Talend Management Console — environments, engines, workspaces, tasks, and plans — and correlates them with the static artifact inventory to mark each artifact as deployed / worker-only / orphaned, with per-environment and prod-presence facts. New `tools/tmc_client.py` (stdlib read-only TMC client) and `tools/tmc_intake.py` (enrichment) drive it; `tools/talend_dependencies.py` adds a dependency/upgrade-risk dimension (jar/lib extraction + version-drift detection across artifacts). A complexity LLM-review triage hook flags artifacts whose deterministic score warrants a manual second look. **Everything in this phase is read-only by construction — no writes to TMC — and was validated live against a real project.** Method documented in `knowledge/tmc/intake-read-only.md`; offline `unittest` coverage in `tests/test_tmc_client.py` and `tests/test_tmc_intake.py`.
- **`/project-intake` — offline Talend project analyzer (phase 1 + 2).** Reads a Talend project on disk and produces an artifact/system/complexity inventory plus interface topology, without contacting any server. Stdlib-only phase-1 pipeline — `tools/talend_item.py` (`.item`/`.properties` XMI parser), `tools/component_catalog.py` (`componentName` → system/direction), `tools/talend_discovery.py` (artifact discovery + classification), `tools/talend_complexity.py` (deterministic, currently uncalibrated complexity metric, emits `calibrated:false` / `"estimated"`), `tools/talend_topology.py` (call graph + interface clustering), orchestrated by `tools/project_intake.py` (CLI: `project_intake.py <path> [--out f.json] [--summary] [--pretty]`). Emits **canonical JSON** with `static` / `tmc` / `manual` provenance per fact; TMC enrichment and manual capture are reserved for later phases (the `tmc{}` and `manual{}` blocks are present but empty). Phase-2 `tools/intake_to_excel.py` (`--in f.json --out f.xlsx`, requires `openpyxl`) renders an optional Excel report. Method documented in `knowledge/patterns/project-intake.md`; mechanics in `knowledge/mechanics/component-system-catalog.md` and `knowledge/mechanics/artifact-detection.md`; the `/project-intake` slash command in `skills/project-intake.md`; offline `unittest` suite under `tests/`. **Validated against a real DI + REST-data-services project** — non-XMI routine/SQL/bean items are no longer XML-parsed, `tRunJob` targets resolve via their `PROJECT:_<id>` repository id, and the component catalog + complexity baseline were hardened (more internal components/prefixes, project-joblet nodes as internal sub-flows, realistic uncalibrated bucket thresholds). ESB route/service detection remains `[VALIDATE]` (no ESB in the validation project).
- **TDS (Talend Data Stewardship) API tooling** — `tools/tds_client.py` (stdlib client: region/base-url, Bearer auth, error parsing, dry-run gate) and `tools/tds_ops.py` (CLI). Real, live-verified CRUD for **data models**, **campaigns** (all 4 task types; RESOLUTION `--demo` template) and **semantic types** (sandbox→draft→publish lifecycle), with dry-run-by-default writes, `--demo` templates, `cimt_demo_<ts>` namespacing, and clean teardown (deletes work despite the docs' "UI-only" claim). **tasks** are creatable/listable via the campaign-scoped endpoint (`task list/get/create`, defaulting `assignee` to `tds.user_email`); **data quality rules** are fully managed via the (undocumented) `rulerepository` service — `dqrule list/get/create/edit/delete/export/apply` (create/edit in the DSEL; `apply` attaches a rule to a data model and tasks then evaluate VALID/INVALID/NOT_APPLICABLE — live-verified). Docs in `knowledge/tds/` (`api-reference.md`, `known-gaps.md`, `demo-workflows.md`); offline `unittest` suite under `tests/`.
